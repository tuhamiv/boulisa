  ...
  profile: z
    .object({
      ...
      nationalId: z
        ...
        const year = (century === 2 ? 1900 : 2000) + Number(nid.substring(1, 3));
        ...
        const sum= weights.reduce((accumulator, currentValue, currentIndex) => accumulator + (currentValue * (digits.at(currentIndex) ?? 0)), 0);
        const remainder= sum % modulus;
        const calcChecksum= (() => {
          switch (remainder) {
            case 0: return 1
            case 1: return 0
            default: return modulus - remainder;
          }
        })();
